...
int findOptimalIndex(int *arr, int *frame_arr, int request_num, int frame_size, int present_index) {
    ...
    for (int i = 0; i < request_num; i++) {
        if (i < frame_size) {
            ...
            if (isPresent(frame_arr, arr[i], 0, frame_size) == -1) {
                int prev_index = findOptimalIndex(arr, frame_arr, request_num, frame_size, i);
