...
int main()
{
    ...
    for (int i = process_num; i > 0;)
    {
        ...
        // printf("No. of available processes at %d: %d\n", present_time, tem);
        // printf("Pnum\tAT\tBT\n");
        // for(int i = 0; i < process_num; i++) {
        //     printf("%d\t%d\t%d\n", i, at[i], bt[i]);
        // }
        int temp_at[tem], temp_bt[tem], iter = 0;
        ...
        // printf("Minimum index in available processes: %d\n", minIndex);
        for (int j = 0; j < process_num; j++)
        {
            if (at[j] == temp_at[minIndex] && bt[j] == temp_bt[minIndex])
            {
                ...
                {
                    present_time += bt[j];
                    // printf("Process %d was completed in time: %d", j, present_time);
                    ct[j] = present_time;
                    bt[j] = -1;
                }
                if (bt[j] <= 0)
    ...
    for (int i = 0; i < process_num; i++)
    {
        ...
        total_tat += tat[i];
        total_wt += wt[i];
        printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\n", pnum[i] - 1, at[i], btb[i], ct[i], tat[i], wt[i], rt[i]);
    }
